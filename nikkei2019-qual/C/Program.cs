...
namespace AtCoder
{
    public class Program
    {
        ...
        {
            var N = ReadInt;
            var AB = new Pair<long, long>[N];

            for (int i = 0; i < N; i++) AB[i] = make_pair(ReadLong, ReadLong);

            Array.Sort(AB, (x, y) =>
            {
                var xx = x.first + x.second;
                var yy = y.first + y.second;
    ...
    public static class ArrayExtensions
    {
            ...
            Func<TSource, TResult> selector2) where TResult : IComparable
        {
            ...
            {
                var result = selector2(y).CompareTo(selector1(x));
                return result != 0 ? result : selector2(y).CompareTo(selector1(x));
            });
        }
    }
    ...
    public class Comparer : IComparer<Pair<int, int>>
    {
        public int Compare(Pair<int, int> x, Pair<int, int> y)
        {
            return x.first == y.first ? x.second.CompareTo(y.second) : x.first.CompareTo(x.first);
        }
    }
