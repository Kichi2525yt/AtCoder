...
namespace AtCoder
{
    public class Program
    {
        ...
        {
            int N = Ri;
            var graph = new List<Pair<int, bool>>[N];
            for (int i = 0; i < N; i++)
            {
                graph[i] = new List<Pair<int, bool>>();
            }
            for (int i = 0; i < N-1; i++)
            {
                int u = Ri-1;
                int v = Ri-1;
                int w = Ri;
                graph[u].Add(make_pair(v, w.IsOdd()));
                graph[v].Add(make_pair(u, w.IsOdd()));
            }
            ...
            var que = new Queue<Pair<int, bool>>();
            que.Enqueue(make_pair(0, false));
            ...
            {
                var p = que.Dequeue();
                foreach (var r in graph[p.first])
                {
                    ...
                    {
                        ans[r.first] = 0;
                        que.Enqueue(make_pair(r.first, false));
                    }
                    else
                    {
                        ans[r.first] = 1;
                        que.Enqueue(make_pair(r.first, true));
                    }
                }
            }
