...
namespace AtCoder
{
    public class Program
    {
        public void Solve()
        {
            int N = ReadInt;
            int K = ReadInt;
            var S = Read.Select(x => x == '1').ToArray();
            ...
            {
                var count = list.Count - K + 1;
                //最初だけ設定しておく
                int max = list[K].first;
                //int start = 0, end = max - 1;
                //PrintDebug(max);
                for (int i = 1; i < count; i++)
                {
                    int nextS, nextE;
                    nextS = list[i - 1].first + list[i - 1].second;
                    nextE = i == count - 1 ? N - 1 : list[i + K].first - 1;
                    chmax(ref max, nextE - nextS + 1);
                    //PrintDebug(nextE - nextS + 1);
                }
