...
namespace AtCoder
{
    public class Program
    {
        private int H, W;
        private bool[][] A;
        bool InMap(int h, int w) => h >= 0 && w >= 0 && h < H && w < W;
        public void Solve()
        {
            H = ReadInt;
            W = ReadInt;
            A = new bool[H][];
            var black = new List<Pair<int, int>>();
            var black_count = 0;
            var values = new int[H][];
            for (int i = 0; i < H; i++) values[i] = Enumerable.Repeat(INF, W).ToArray();
            for (int i = 0; i < H; i++)
            {
                var a = Read;
                A[i] = new bool[W];
            ...
            {
                //それぞれの4辺を調べて値を更新
                var next = new List<Pair<int, int>>();
                foreach (var b in black)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (values[nextH][nextW] == INF)
                        {
                            black_count++;
                            next.Add(make_pair(nextH, nextW));
                        }
                        chmin(ref values[nextH][nextW], cur);
                    }
                }
